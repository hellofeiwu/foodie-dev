...
public class SSOController {
     ...
     * CAS的统一登录接口剖
     *      目的：
     *          1. 登录后创建用户的全局会话token             -> uniqueToken
     *          2. 创建用户的全局门票，用以表示在CAS端是否登录  -> userTicket
    ...
    public String doLogin(String username,
        ...
        redisOperator.set(REDIS_USER_TOKEN + ":" + result.getId(), JsonUtils.objectToJson(usersVO));

        // 3.生成全局门票，代表用户在 CAS 端登录过
        String userTicket = UUID.randomUUID().toString().trim(); // 这里加trim是因为第三方工具有时候会在结尾加空格
